...
public class LuceneEngineIT extends KNNRestTestCase {
    ...
    public void testQuery_multipleEngines() throws Exception {
        ...
        String jvectorField = "jvector-field";
        SpaceType jvectorSpaceType = SpaceType.L2;
        ...
        XContentBuilder builder = XContentFactory.jsonBuilder()
            ...
            .endObject()
            .endObject()
            .startObject(jvectorField)
            .field(TYPE_FIELD_NAME, KNN_VECTOR_TYPE)
            .field(DIMENSION_FIELD_NAME, DIMENSION)
            .startObject(KNNConstants.KNN_METHOD)
            .field(KNNConstants.NAME, DISK_ANN)
            .field(KNNConstants.METHOD_PARAMETER_SPACE_TYPE, jvectorSpaceType.getValue())
    ...
    /**
     * Tests the search functionality of a KNN index when using a non-Lucene engine with a filter query applied.
     * <p>
     * This test performs the following:
     * 1. Creates a KNN index with specified mapping details using the non-Lucene engine (e.g., JVECTOR).
     *    - Specifies KNN method, dimensions, and search space type.
     * 2. Indexes documents with KNN vector fields and associated attributes such as "color" and "taste".
     * 3. Executes a KNN query with a filter condition (`termQuery`) applied.
     *    - The filter ensures that only documents with a specific attribute value (e.g., "red" for color field) are considered in the search.
     *    - Uses the SpaceType.L2 distance metric for retrieving results.
     * 4. Validates the response:
     *    - Ensures that the search accommodates filtering, excluding results not satisfying the filter condition.
     *    - Verifies the number of results and checks the order of results based on proximity of target search vector to indexed document vectors.
     * <p>
     * This test emphasizes ensuring compatibility and correct behavior of KNN search under a non-Lucene engine while respecting filter criteria.
     *
     * @throws Exception if an error occurs during the creation of the index, document addition, or query execution.
     */
    public void testQuery_filterWithNonLuceneEngine() throws Exception {
        XContentBuilder builder = XContentFactory.jsonBuilder()
            ...
            .field(KNNConstants.NAME, DISK_ANN)
            .field(KNNConstants.METHOD_PARAMETER_SPACE_TYPE, SpaceType.L2.getValue())
            .field(KNNConstants.KNN_ENGINE, JVECTOR_NAME)
            .endObject()
            .endObject()
            .endObject()
            .endObject();
        ...
        // the target search vector should be closest to DOC_ID_2 that has a color field "green" with L2 of 0
        // However the filter rules out this result which leaves us with the second and third-furthest results with color field "red"
        final float[] searchVector = { 3.2f, 2.1f, 4.8f };
        int k = 3;
        var response = searchKNNIndex(
            INDEX_NAME,
        ...
        Assert.assertEquals(DOC_ID_3, results.getFirst().getDocId());
    }
