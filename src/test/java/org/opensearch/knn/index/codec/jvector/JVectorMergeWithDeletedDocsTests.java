...
public class JVectorMergeWithDeletedDocsTests extends LuceneTestCase {
     ...
     */
    @Test
    public void testMergesWithLeadingSegment() throws IOException {
        final Map<String, float[]> docs = new HashMap<>();
        final int dimension = 64;
        final int k = 11;
        ...
        try (FSDirectory dir = FSDirectory.open(indexPath); IndexWriter writer = new IndexWriter(dir, config)) {
            ...
            // Segment 1: 10 documents
            log.info("Creating segment 1: 10 docs");
            for (int i = 0; i < 10; i++) {
                Document doc = new Document();
                float[] vector = new float[dimension];
                Arrays.fill(vector, docId * random().nextFloat(1.0f));
            ...
            // Segment 2: 1 document
            log.info("Creating segment 2: 1 doc");
            for (int i = 10; i < 11; i++) {
                Document doc = new Document();
                float[] vector = new float[dimension];
                Arrays.fill(vector, docId * random().nextFloat(1.0f));
     ...
     */
    @Test
    public void testMergesWithLeadingSegmentAndDeletedDocs() throws IOException {
        final Map<String, float[]> docs = new HashMap<>();
        final int dimension = 64;
        final int k = 11;
        ...
        try (FSDirectory dir = FSDirectory.open(indexPath); IndexWriter writer = new IndexWriter(dir, config)) {
            ...
            // Segment 3: Delete 1 document
            log.info("Creating segment 3: Delete 1 doc");
            writer.deleteDocuments(new Term(TEST_ID_FIELD, String.valueOf(randomFrom(0, 4, 7, 10))));
            writer.commit();
