...
public final class FaissKnnVectorsWriter extends KnnVectorsWriter {
    ...
    for (Map.Entry<FieldInfo, FlatFieldVectorsWriter<?>> entry : rawFields.entrySet()) {
      ...
      switch (fieldInfo.getVectorEncoding()) {
        ...
        case FLOAT32 -> {
          ...
          DocsWithFieldSet docsWithFieldSet = rawWriter.getDocsWithFieldSet();

          writeFloatField(
              fieldInfo,
              new BufferedFloatVectorValues(vectors, dimension, docsWithFieldSet),
  ...
  private static class BufferedFloatVectorValues extends FloatVectorValues {
    ...
    BufferedFloatVectorValues(
        List<float[]> floats, int dimension, DocsWithFieldSet docsWithFieldSet) {
      this.floats = floats;
    ...
    public float[] vectorValue(int targetOrd) {
      return floats.get(targetOrd);
    }

    @Override
    public DocIndexIterator iterator() {
      return new DocIndexIterator() {
        private final DocIdSetIterator delegate = docsWithFieldSet.iterator();
        private int index = -1;
